...
export async function getTicketsForUser(userId: string): Promise<Ticket[]> {
  ...
  const { data, error } = await admin
    ...
    .select(
      ...
      "id, status, checked_in_at, created_at, events(id, name, day, start_time, end_time, venue, is_entry_pass, spans_both_days, clubs!events_club_id_fkey(name)), payments(status, amount_inr)",
    )
    .eq("user_id", userId)
